"""
ProbeNpwn v3.3.0.1 - Ultimate handshake/PMKID capture plugin
Author: AlienMajik
License: GPL3
New in 3.3.0.1:
- Fix for external tool calling
...
} #  or add sbin and bin to the path via OS 
...
class ProbeNpwn(plugins.Plugin):
    ...
    __github__ = 'https://github.com/AlienMajik/pwnagotchi_plugins/blob/main/probenpwn.py'
    def __init__(self):
        super().__init__()
    ...
    def _get_tool_path(self, name: str):
    ...
    def _check_tool(self, name: str) -> bool:
        try:
            path = shutil.which(name)
            # If not found, use hardcoded path
            if not path:
                path = TOOL_PATHS.get(name)

...
class pwncrackV2(plugins.Plugin):
    ...
    def on_loaded(self):
        logging.info('[pwncrackV2] loading')
        self.ready = True
    ...
    def on_internet_available(self, agent):
        ...
        if remaining_wait_time > 0:
            logging.debug(f"[pwncrackV2] Waiting {remaining_wait_time:.1f} more seconds before next run.")
            return
        ...
        logging.info(f"[pwncrackV2] Running upload process. waiting: {self.timewait} seconds.")
        try:
            self._convert_and_upload()
            self._download_potfile()
            self.last_run_time = current_time
        except Exception as e:
            logging.error(f"[pwncrackV2] Error occurred during upload process: {e}", exc_info=True)
    ...
    def _convert_and_upload(self):
        ...
        pcap_files = [f for f in os.listdir(self.handshake_dir)
                      if f.endswith('.pcap') and os.path.getmtime(os.path.join(self.handshake_dir,f)) > last_up_time]
        
        pcap_files = remove_whitelisted(all_pcaps, config['main']['whitelist']) # remove any whitelisted pcap files 
        ...
        if pcap_files:
            ...
            # Log the response
            logging.info(f"[pwncrackV2] Upload response: {response.json()}")
            ...
            logging.info("[pwncrackV2] No .pcap files found to convert (or all files are whitelisted).")
    ...
    def _download_potfile(self):
        ...
        if response.status_code == 200:
            ...
            logging.info(f"[pwncrackV2] Potfile downloaded to {self.potfile_path}")
        else:
            logging.error(f"[pwncrackV2] Failed to download potfile: {response.status_code}")
            logging.error(f"[pwncrackV2] {response.json()}")  # Log the error message from the server

    def on_unload(self, ui):
        logging.info('[pwncrackV2] unloading')
    ...
    def on_webhook(self, path, request):
        ...
        try:
            ...
            logging.error(f"[pwncrackV2] {repr(e)}")

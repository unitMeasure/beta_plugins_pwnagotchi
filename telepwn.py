...
class TelePwn(plugins.Plugin):
    __author__ = "WPA2"
    __version__ = "2.0.0.1"
    __github__ = "https://github.com/wpa-2/Pwnagotchi-Plugins/blob/main/TelePwn/telepwn.py"
    __license__ = "GPL3"
    ...
    def load_config(self):
        try:
            with open(CONFIG_FILE, "r") as f:
                config = toml.load(f)
                raw_rotation = int(config.get("ui", {}).get("display", {}).get("rotation", 0))
                self.screen_rotation = (360 - raw_rotation) % 360
